...
async def math_eval(solution: str, ground_truth: str, backend):
    try:
        answer_match = re.search(r'\\boxed\{([^}]+)\}', solution)
        answer = answer_match.group(1).strip() if answer_match else ""
        result = await backend.llm_aask(f"Please evaluate if the following answer is correct: {answer}. The correct answer is: {ground_truth}. Please respond with 'Yes' or 'No'.")
        logger.info(f"Evaluation response from backend: {result}")
...
async def run_correctness_eval_task(
    task: dict, semaphore: asyncio.Semaphore = None, backend = None
) -> tuple[str, bool]:
    """Run one task evaluation and return ``(task_id, passed)``."""
    async with semaphore:
        solution = task["model_prediction"]
        ground_truth = task["ground_truth"]
        result = await math_eval(solution, ground_truth, backend)
...
async def run_eval_tasks(
    ...
    coros = [run_correctness_eval_task(task, semaphore, backend=backend) for task in tasks]
